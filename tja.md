...
`E` | Top-single bottom-single note | Green or blue circle on the top lane and red circle on the bottom lane, joined by grey background | [Correctly input](#judgement-expection-type) either Bottom + either Top with the first input within the GOOD‡ timing window. <br /> Awards the same as `1` according to the timing of the first input. | Otherwise, correctly, [incompletely, or wrongly input](#judgement-expection-type) with the first input within the MISS judgment window (consumes the note) or [missing input](#judgement-expection-type) (keeps note's visibility). <br /> Gives the same penalty as `1`. |
? | Top-single bottom-double note | Green or blue circle on the top lane and red square on the bottom lane, joined by grey background | [Correctly input](#judgement-expection-type) Bottom-left + Bottom-top + either Top with the first input within the GOOD‡ timing window, consumes the note. <br /> Awards the same as `1` according to the timing of the first input. | Otherwise, correctly, [incompletely, or wrongly input](#judgement-expection-type) with the first input within the MISS judgment window (consumes the note) or [missing input](#judgement-expection-type). <br /> Gives the same penalty as `1`. | No note symbols have been assigned.
? | Top-double bottom-single note | Green or blue square on the top lane and red circle on the bottom lane, joined by grey background | [Correctly input](#judgement-expection-type) either Bottom + Top-left + Top-right with the first input within the GOOD‡ timing window, consumes the note. <br /> Awards the same as `1` according to the timing of the first input. | Otherwise, correctly, [incompletely, or wrongly input](#judgement-expection-type) with the first input within the MISS judgment window (consumes the note) or [missing input](#judgement-expection-type). <br /> Gives the same penalty as `1`. | No note symbols have been assigned.
`G` | Top-double bottom-double note | Green or blue square on the top lane and red square on the bottom lane, joined by grey background | [Correctly input](#judgement-expection-type) Bottom-left + Bottom-top + Top-left + Top-right with the first input within the GOOD‡ timing window, consumes the note. <br /> Awards the same as `1` according to the timing of the first input. | Otherwise, correctly, [incompletely, or wrongly input](#judgement-expection-type) with the first input within the MISS judgment window (consumes the note) or [missing input](#judgement-expection-type). <br /> Gives the same penalty as `1`. |
...
`6` | Head of both bar drumroll <br /> Examples: `6008`, `6668`, `6001` | Red circle on the bottom lane and green circle on the top lane, each with bar attached behind on the respective lane, joined by grey background | Repeatly [correctly input](#judgement-expection-type) either Bottom or either Top by any times during its duration, reacts to each input up to *unspecified* speed††. | (impossible to fail) | By analogy with `GAME:Konga`.
`H` | (same as `6`) <br /> Examples: `H008`, `HHH8`, `H001` | (see `6`) | (see `6`) | (see `6`) | By analogy with `GAME:Konga`. |
`7` | Head of regular *<ruby>激<rt>Geki</rt>連<rt>ren</rt>打<rt>da</rt></ruby>/<ruby>ゲ<rt>Ge</rt>キ<rt>ki</rt>連<rt>ren</rt>打<rt>da</rt></ruby>* "fierce drumroll" burst note / *<ruby>風<rt>Fuu</rt>船<rt>sen</rt></ruby>/<ruby>ふ <rt>Fu</rt>う<rt>u</rt>せ<rt>se</rt>ん<rt>n</rt></ruby>* balloon <br /> Examples: `7008`, `7778`, `7001` | Small orange circle (slightly brighter than `1`) with orange-ish red balloon attached behind | Repeatly [correctly input](#judgement-expection-type) either Bottom until exactly certain amount of reacted inputs during its duration, reacts to each correct input up to *unspecified* speed††. [Consumes wrong inputs](#judgement-expection-type). Consumes the note when cleared. | Fail to reach enough amount of reacted inputs. <br /> Does not give penalties except that notes (except bombs/mines (?)) placed within the duration of the balloon are impossible to press while the balloon is not cleared. | By analogy with Taiko mode.
`9` | Head of special burst note/balloon <br /> Examples: `9008`, `9998`, `9001` | (Vary) | (Vary) <br> Repeatly [correctly input](#judgement-expection-type) either Bottom until exactly certain amount of reacted inputs (shared among players) during its duration, reacts to each correct input up *unspecified* speed††. [Consumes wrong inputs](#judgement-expection-type). Consumes the note when cleared. | (same as `7`) | Might become `7` when not all players encounter `9` with the note head, the full bonus time point, & the note end respectively at the same time position for each player. <br /> Not in the official games. <br /> By analogy with Taiko mode.
...
`8` | Explicit end of a drumroll-type note (if any), otherwise (blank) | &bull; (round end of a bar) (end of a bar-type drumroll) <br /> &bull; (none) (otherwise) | Nothing needs to be done. Consumes no input. | (impossible to fail) | Stop repeately pressing buttons non-after the point (end of drumrolls)
